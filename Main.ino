...
void loop() {
	...
	if(above_thresh) {
		...
		if(inout.getCircState() && (millis() - inout.getLastCircAction()) >= (variables[ON_VAR] * 60000)) {
			inout.circOff();
		} else if(!inout.getCircState() && (millis() - inout.getLastCircAction()) >= (variables[OFF_VAR] * 60000)) {
			inout.circOn();
		}
	}
